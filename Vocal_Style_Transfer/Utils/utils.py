...
def load_wavs(file_path, sr) :
    ...
    print('files')
    print(files)
    # wav Загрузить файл (перейти на генератор)
...
def world_encode_data(wavs, fs, frame_period = 5.0, coded_dim = 24):
    ...
    print('Encoding....... Это занимает много времени .... Пожалуйста, подождите..... ')
    for i, encode in enumerate(decompose_generator):
        coded_sp = world_encode_spectral_envelop(sp=encode[2], fs=fs, dim=coded_dim)
        f0s.append(encode[0])
        coded_sps.append(coded_sp)
        print('{}Кодирование 1-го файла завершено'.format(i))

    print('==== Кодирование всех файлов ====')
    return f0s, coded_sps


def transpose_in_list(lst):

    # I can do this quickly, but first pass
    transposed_lst = list()
    for array in lst:
...
# Внизу тестовая часть декодирования и составная часть пройдены.
